...
$finished_update = $dbh->prepare("UPDATE zipcodes SET LastUpdate = ? WHERE ZipCode = ?;");
...
foreach($dbh->query("SELECT ZipCode, State FROM zipcodes WHERE ZipCode = '".$zip."' AND STATE = '".$state."';") as $zipcode_row){
    ...
    $finished_update->execute(array($now, $zipcode_row['ZipCode']));
